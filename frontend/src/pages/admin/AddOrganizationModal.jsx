...
const AddOrganizationModal = ({ isOpen, onClose, onOrganizationCreated }) => {
  const [name, setName] = useState("");
  const [code, setCode] = useState("");
  const [country, setCountry] = useState("");
  const [status, setStatus] = useState("1"); // Default to Active (using string for consistency)
  ...
  const [isLoading, setIsLoading] = useState(false); // Added loading state for create button
  ...
  const fetchCountries = async () => {
    ...
    try {
      ...
      setCountries(data.country || []); // Ensure we have an array
    } catch (error) {
  ...
  const handleCreate = async () => {
    ...
    const newOrganization = {
      ...
      code: code,
      country: country,
      status: parseInt(status, 10), // Parse status to integer
    ...
    try {
      ...
      if (response.ok) {
        setSuccessMessage(data.message || "Organization created successfully!");
        setName("");
        setCode("");
        setCountry("");
        setStatus("1");
        setDescription("");
        if (onOrganizationCreated && data.organization) {
          onOrganizationCreated(data.organization);
        }
        setTimeout(onClose, 1500); // Close after a short delay
      } else {
        setErrorMessage(data.message || "Failed to create organization.");
      }
    } catch (error) {
