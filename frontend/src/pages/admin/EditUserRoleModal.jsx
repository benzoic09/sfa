...
const EditUserRoleModal = ({ isOpen, onClose, role, onRoleUpdated }) => {
  ...
  // Populate form with role data when the modal is opened and role prop changes
  useEffect(() => {
    if (isOpen && role) {
      setRoleName(role.name);
      setDescription(role.description || "");
      setStatus(role.status ? "1" : "0"); // Assuming 1 for Active, 0 for Inactive
      setErrorMessage("");
      setSuccessMessage("");
    } else if (!isOpen) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!role) {
      console.error("No role data provided for editing.");
      setErrorMessage("No role data provided.");
      setIsLoading(false);
      return;
    }
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/admin/roles/${role.id}`,
        {
      ...
      if (response.ok) {
        ...
        if (onRoleUpdated && data.role) {
          onRoleUpdated(data.role); // Pass the updated role data back
        }
